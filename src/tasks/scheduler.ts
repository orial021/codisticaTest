...
class Scheduler {
  ...
  async runAllTasks(): Promise<void> {
    const taskResults: { [key: number]: string } = {};
  ...
  private async runTask(task: Task, taskResults: { [key: number]: string }): Promise<void> {
  ...
  private markFailedTasks(taskId: number, taskResults: { [key: number]: string }): void {
    const queue = [taskId];
